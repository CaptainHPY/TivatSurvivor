...
class Animition {
public:
	Animition(Atlas* atlas, int interval) {
		anim_atlas = atlas;
...
public:
	Player() {
		...
		anim_left = new Animition(atlas_player_left, 90);
		anim_right = new Animition(atlas_player_right, 90);
	}
	~Player() {
		delete anim_left;
		delete anim_right;
...
public:
	Enemy() {
		...
		anim_left = new Animition(atlas_enemy_left, 90);
		anim_right = new Animition(atlas_enemy_right, 90);
	...
	bool CheckBulletCollision(const Bullet& bullet) {
		bool is_over_lap_x = bullet.position.x >= position.x && bullet.position.x <= position.x + FRAME_WIDTH;
		bool is_over_lap_y = bullet.position.y >= position.y && bullet.position.y <= position.y + FRAME_HEIGHT;
		return is_over_lap_x && is_over_lap_y;
	}
	bool CheckPlayerCollision(const Player& player) {
		POINT check_position = { position.x + FRAME_WIDTH / 2,position.y + FRAME_HEIGHT / 2 };
		POINT player_position = player.GetPosition();
		bool is_over_lap_x = check_position.x >= player_position.x && check_position.x <= player_position.x + player.FRAME_WIDTH;
		bool is_over_lap_y = check_position.y >= player_position.y && check_position.y <= player_position.y + player.FRAME_HEIGHT;
		return is_over_lap_x && is_over_lap_y;
	}
